...
from ldm.modules.diffusionmodules.my_decoder_large2 import Decoder, Decoder_wocondition
...
class AutoencoderKL(pl.LightningModule):
    def __init__(self,
        ...
        self.decoder = Decoder(**ddconfig)
        self.scheduler_config = scheduler_config
        self.scheduler_config_d = scheduler_config_d
        self.loss = instantiate_from_config(lossconfig)
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            ...
            if dist.get_rank() == 0:
                ...
                wandb.log({"lr_ae_pre": self.optimizers()[0].state_dict()['param_groups'][0]["lr"],
                           "lr_ae": self.optimizers()[0].state_dict()['param_groups'][1]["lr"]})
            return aeloss
        ...
        if optimizer_idx == 1:
            ...
            if dist.get_rank() == 0:
                ...
                wandb.log({"lr_d": self.optimizers()[1].state_dict()['param_groups'][0]["lr"]})
            return discloss
    ...
    def configure_optimizers(self):
        ...
        opt_disc = torch.optim.Adam(self.loss.discriminator.parameters(),
                                    lr=lr, betas=(0.5, 0.9))
        num_batches = int(1152196 / (self.num_gpus  * self.train_dataloader().batch_size))
        total_step = int(self.trainer.max_epochs * num_batches)
        print(f"The total number of steps is {total_step}")
        if self.scheduler_config is not None:
            ...
            self.scheduler_config_d["params"]["start_step"] = self.loss.discriminator_iter_start
            scheduler = instantiate_from_config(self.scheduler_config)
            scheduler_d = instantiate_from_config(self.scheduler_config_d)
            scheduler_pre = instantiate_from_config(pre_trained_scheduler_config)

            print("Setting up LambdaLR scheduler...")
            scheduler = [
                {
                    'scheduler': LambdaLR(opt_ae, lr_lambda=[scheduler_pre.schedule, scheduler.schedule]),
                    'interval': 'step',
                    'frequency': 1
                },
                {
                    'scheduler': LambdaLR(opt_disc, lr_lambda=scheduler_d.schedule),
                    'interval': 'step',
                    'frequency': 1
                },
            ]
    ...
    def log_images(self, batch, only_inputs=False, **kwargs):
        log = dict()
        x, mask = self.get_input(batch, self.image_key)
        image = x.clone()
        if not only_inputs:
            xrec, posterior = self([x, mask])
            if x.shape[1] > 3:
                # colorize with random projection
                assert xrec.shape[1] > 3
                x = self.to_rgb(x)
                xrec = self.to_rgb(xrec)
            log["samples"] = self.decode(torch.randn_like(posterior.sample()), image, mask)
            log["reconstructions"] = xrec
        log["inputs"] = x
        return log
...
class AutoencoderKL_wocondition(pl.LightningModule):
    def __init__(self,
                 ddconfig,
                 lossconfig,
                 embed_dim,
        ...
        self.image_key = image_key
        self.encoder = Encoder(**ddconfig)
        self.decoder = Decoder_wocondition(**ddconfig)
        self.scheduler_config = scheduler_config
        self.scheduler_config_d = scheduler_config_d
        self.loss = instantiate_from_config(lossconfig)
        assert ddconfig["double_z"]
